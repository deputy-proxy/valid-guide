...
it('snapshots the service package and price on the request', function () {
    $request = commercialTermsRequest();
    $package = commercialTermsPackage();

    $updated = app(EvaluationRequestCommercialTerms::class)
        ->applyPackage($request, $package, 'standard');

    expect($updated->service_package_id)->toBe($package->id)
        ->and($updated->service_package_name_snapshot)->toBe('Standard Evaluation')
        ->and($updated->quoted_price)->toBe('500.00')
        ->and($updated->currency)->toBe('EUR')
        ->and($updated->complexity)->toBe('standard');
});

it('does not allow commercial terms to change once payment has started', function () {
    $request = commercialTermsRequest();
    $package = commercialTermsPackage();
...
it('preserves the request price when the catalog package changes', function () {
    $request = commercialTermsRequest();
    $package = commercialTermsPackage();
    $request = app(EvaluationRequestCommercialTerms::class)->applyPackage($request, $package, 'standard');
    app(EvaluationRequestStateTransition::class)
    ...
    expect($request->refresh()->quoted_price)->toBe('500.00')
        ->and($request->service_package_terms_snapshot['price'])->toBe('500.00');
});

it('rejects inactive service packages', function () {
    $request = commercialTermsRequest();
    $package = commercialTermsPackage();
    $package->status = 'inactive';
    $package->save();

    expect(fn () => app(EvaluationRequestCommercialTerms::class)
        ->applyPackage($request, $package, 'standard'))
        ->toThrow(DomainStateTransitionException::class);
});
