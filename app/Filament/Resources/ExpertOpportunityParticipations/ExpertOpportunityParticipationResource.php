...
namespace App\Filament\Resources\ExpertOpportunityParticipations;

use App\Enums\ExpertOpportunityParticipationStatus;
use App\Filament\Resources\ExpertOpportunityParticipations\Pages\ListExpertOpportunityParticipations;
...
final class ExpertOpportunityParticipationResource extends Resource
{
    ...
    public static function canAccess(): bool { return self::user()->isPlatformAdmin(); }
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            Action::make('cancel')->color('danger')->visible(fn (ExpertOpportunityParticipation $record): bool => in_array($record->status, [ExpertOpportunityParticipationStatus::Selected, ExpertOpportunityParticipationStatus::Accepted], true))->form([Textarea::make('reason')->required()->minLength(3)->maxLength(2000)])->action(fn (ExpertOpportunityParticipation $record, array $data) => self::run(fn () => app(ExpertOpportunityParticipationService::class)->cancel($record, self::user(), (string) $data['reason']), 'Engagement cancelled')),
    ...
    public static function getPages(): array { return ['index' => ListExpertOpportunityParticipations::route('/')]; }
    ...
    private static function run(callable $callback, string $success): void
    {
        ...
        catch (DomainStateTransitionException $exception) { Notification::make()->danger()->title('Action blocked')->body($exception->getMessage())->send(); }
