...
function evaluationRequestLifecycleFixture(): EvaluationRequest
{
    ...
    return EvaluationRequest::create([
        'organization_id' => $organization->id, 'product_id' => $product->id, 'service_package_id' => $package->id,
        'service_package' => $package->slug, 'service_package_name_snapshot' => $package->name,
        'service_package_description_snapshot' => $package->description, 'complexity' => 'standard',
        'quoted_price' => $package->price, 'currency' => $package->currency,
    ]);
}
...
test('state transition service rechecks the locked current state', function () {
    ...
    expect(fn () => app(EvaluationRequestStateTransition::class)->transition($request, EvaluationRequestStatus::AwaitingPayment))->toThrow(DomainStateTransitionException::class);
});
