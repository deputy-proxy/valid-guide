...
final class ExpertOpportunityParticipationService
{
    public function apply(ExpertOpportunity $opportunity, User $expert, string $disclosure): ExpertOpportunityParticipation
    {
        ...
        if (trim($disclosure) === '') {
            throw new DomainStateTransitionException('A conflict-of-interest disclosure is required.');
        }
        if ($opportunity->status !== ExpertOpportunityStatus::Published || ($opportunity->application_deadline !== null && $opportunity->application_deadline->isPast())) {
            throw new DomainStateTransitionException('This opportunity is not accepting applications.');
        }
        if ($opportunity->participations()->where('auditor_profile_id', $profile->getKey())->exists()) {
            throw new DomainStateTransitionException('The Expert has already applied to this opportunity.');
        }

        return DB::transaction(function () use ($opportunity, $profile, $expert, $disclosure): ExpertOpportunityParticipation {
            $participation = ExpertOpportunityParticipation::query()->create([
                'expert_opportunity_id' => $opportunity->getKey(),
                'auditor_profile_id' => $profile->getKey(),
                'status' => ExpertOpportunityParticipationStatus::Applied,
                'conflict_disclosure' => trim($disclosure),
                'applied_at' => now(),
            ]);
    ...
    public function withdraw(ExpertOpportunityParticipation $participation, User $expert): ExpertOpportunityParticipation
    {
        ...
        if ($participation->status !== ExpertOpportunityParticipationStatus::Applied) {
            throw new DomainStateTransitionException('Only active applications can be withdrawn.');
        }
    ...
    {
        $this->admin($actor);
        if ($participation->conflict_determined_at !== null || ! in_array($outcome, ['cleared', 'conflicted'], true)) {
            throw new DomainStateTransitionException('The conflict determination cannot be changed.');
        }
    ...
    public function reject(ExpertOpportunityParticipation $participation, User $actor, string $reason): ExpertOpportunityParticipation
    {
        ...
        return $this->review($participation, ExpertOpportunityParticipationStatus::Rejected, $reason, $actor);
    }
    ...
    public function cancel(ExpertOpportunityParticipation $participation, User $actor, string $reason): ExpertOpportunityParticipation
    {
        ...
        return $this->review($participation, ExpertOpportunityParticipationStatus::Cancelled, $reason, $actor);
    }

    public function complete(ExpertOpportunityParticipation $participation, User $actor): ExpertOpportunityParticipation
    {
        $this->admin($actor);
        if ($participation->status !== ExpertOpportunityParticipationStatus::Accepted) {
            throw new DomainStateTransitionException('Only accepted engagements can be completed.');
        }
    ...
    {
        $reason = trim($reason);
        if ($reason === '' || ! in_array($participation->status, [ExpertOpportunityParticipationStatus::Applied, ExpertOpportunityParticipationStatus::Selected, ExpertOpportunityParticipationStatus::Accepted], true)) {
            throw new DomainStateTransitionException('The participation cannot be changed.');
        }
    ...
    private function eligible(User $expert, ExpertOpportunity $opportunity): AuditorProfile
    {
        ...
        if (($constraints['methodology_literate'] ?? false) === true && $profile->methodology_literate !== true) {
            throw new DomainStateTransitionException('The opportunity requires methodology literacy.');
        }
        $requiredTypes = array_values(array_filter($opportunity->product_types ?? [], 'is_string'));
        if ($requiredTypes !== [] && array_intersect($requiredTypes, $profile->format_experience ?? []) === []) {
            throw new DomainStateTransitionException('The Expert does not match the required product type experience.');
        }
        ...
        if ($requiredAreas !== []) {
            ...
            if (array_intersect(array_map('strtolower', $requiredAreas), $topics) === []) {
                throw new DomainStateTransitionException('The Expert does not match the required verified expertise.');
            }
        }
    ...
    private function admin(User $actor): void
    {
        if (! $actor->isPlatformAdmin()) {
            throw new DomainStateTransitionException('Only platform administrators can govern opportunity participation.');
        }
    }
}
