...
it('supports acceptance and completion', function () {
    $admin = opportunityAdmin(); $opportunity = opportunityDraft($admin); app(\App\Services\ExpertOpportunityGovernance::class)->publish($opportunity, $admin);
    $expert = opportunityExpert(); $service = app(ExpertOpportunityParticipationService::class);
    $participation = $service->apply($opportunity, $expert, 'Clear.');
    $service->determineConflict($participation, $admin, 'cleared'); $service->select($participation, $admin); $service->accept($participation, $expert);
...
it('prevents another expert from withdrawing an application', function () {
    $admin = opportunityAdmin(); $opportunity = opportunityDraft($admin); app(\App\Services\ExpertOpportunityGovernance::class)->publish($opportunity, $admin);
    $expert = opportunityExpert(); $other = opportunityExpert();
