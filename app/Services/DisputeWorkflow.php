...
class DisputeWorkflow
{
    public function submit(
        ...
        if (
            $normalizedGrounds === []
            || collect($normalizedGrounds)->contains(
                fn (string $ground): bool => DisputeGround::tryFrom($ground) === null,
            )
        ) {
    ...
    private function isOriginalParticipant(Evaluation $evaluation, User $user): bool
    {
        return $evaluation->assignments()->where('auditor_id', $user->id)->exists()
            ...
            || $evaluation->findings()
                ->whereHas('auditorEvaluation.assignment', fn ($query) => $query->where('auditor_id', $user->id))
                ->exists();
    }
